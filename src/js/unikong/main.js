...
StackExchange.Unikong.generateLeaderboardScores = function(group, addNew) {
    ...
    if (group) {
        for (var i = 0; i < scoreCount; i++) {
            ...
            var fillColor = result.initials === "YOU" ? 0xF48024 : 0xFFFFFF;
            
            var scoreString = result.score.toString();
            while (scoreString.length < maxScoreLength) scoreString = " " + scoreString;

            var scoreEntry = this.game.add.retroFont('font', 13, 13, Phaser.RetroFont.TEXT_SET3, 36, 2);

            var img = this.game.add.image(this.game.world.centerX, this.game.world.centerY - 115 + (25 * i), scoreEntry);
            scoreEntry.text = result.initials + "    " + scoreString;
            img.tint = fillColor;
...
var creditsSectionFontStyle = { font: 'Comic Sans MS', fontSize: '20px', fill: '#fff' };
var creditsNameFontStyle = { font: 'Comic Sans MS', fontSize: '16px', fill: '#fff' };
